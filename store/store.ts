...
const useStore = create<GameState>((set, get) => ({
  ...
  setTargetWeight: (target: number) =>
    set((_state) => ({ targetWeight: target })),
  ...
  removePlayer: (id: number) =>
    set(({ players }) => ({
      players: players.filter(({ id: pid }) => id !== pid),
    })),
  getPlayer: (id: number) => get().players.find(({ id: pid }) => pid == id),
  setPlayer: (id: number, player: GamePlayer) =>
    set(({ players }) => ({
      players: players.map((p) => (id === p.id ? player : p)),
    })),
  ...
  setPlayerWeight: (id: number, weight: number) =>
    set(({ players }) => ({
      players: players.map((p) => ({
        weight: id === p.id ? weight : p.weight,
        id: p.id,
        points: p.points,
        name: p.name,
      })),
  ...
  decrementRounds: () =>
    set(({ leftRounds }) => ({ leftRounds: leftRounds - 1 })),
}));
